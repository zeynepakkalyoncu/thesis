...
Our best NDCG@20 on \mbox{Robust04} at 0.5325 approaches their results with CEDR-KNRM even though we optimize for AP instead of NDCG@20.
...
Figure \ref{fig:perquery-msmarcomb} plots the $\Delta$ AP per query between our best model, $ \textrm{BERT}(\textrm{MS MARCO}\rightarrow\textrm{MB}) $, and the baseline sorted in descending order.
...
The average AP for the BM25+RM3 baseline drops by 29\% from a single token query to a query with four tokens, indicating that exact term-matching is more effective for fewer number of terms.
...
Furthermore, the effectiveness of the best models  $ \textrm{BERT}(\textrm{MS MARCO}\rightarrow\textrm{MB}) $ and $ \textrm{BERT}(\textrm{CAR}\rightarrow\textrm{MB}) $ degrade much less in comparison, at only 12\% and 5\% respectively, showing that they are indeed able to use contextualized query representations most effectively.

Note that all BERT-based models outperform the baseline across all queries.
While the gains introduced by BERT are statistically significant for query length 2 and 3 for all models, the difference for single word queries is only significant for $ \textrm{BERT}(\textrm{MB}) $, which suggests that it may not be possible to draw generalizable conclusions from this observation due to the small sample size for query length 1.
...
Considering any number of top scoring sentences, both $ \textrm{BERT}(\textrm{CAR}) $ and $ \textrm{BERT}(\textrm{MS MARCO}) $ outperform the BM25+RM3 baseline by only 0.012 AP on Robust04 with similar gains in P@20 and NDCG@20.
...
For each condition, we re-tuned the weights in Equation~\ref{eq1} to maximize AP on the development fold.
