...
Tables \ref{tab:results-robust04}, \ref{tab:results-core17}, and \ref{tab:results-core18} display our main results on Robust04, Core17 and Core18.
...
The $ n $S preceding the model name indicates that the top $ n $ sentences in each document are interpolated to compute an overall document score.
...
By fine-tuning BERT on three different datasets alone and in combination, we study the effect of training data on the power of our learned relevance matching model.
...
We hypothesize that CAR might have a similar effect to language pre-training in that it does not directly apply to the downstream document retrieval task, but provides a better representation that can benefit from fine-tuning on MB.
...
Preliminary experiments show that, in general, looking beyond the top three sentence does not help effectiveness.
...
The meta-analysis\footnote{\url{https://github.com/lintool/robust04-analysis}} of over 100 papers up until 2019 by Yang et al. \cite{Yang_etal_SIGIR2019} currently provides the most thorough overview of related work on Robust04.
...
Furthermore, our results also exceed the highest known score of 0.3686, which is a non-neural method based on ensembles~\cite{Cormack:2009:RRF:1571941.1572114}.
...
Figure \ref{fig:perquery-msmarcomb} plots the change in AP per query between $ \textrm{BERT}(\textrm{MS MARCO}\rightarrow\textrm{MB}) $ and the baseline sorted in descending order.
...
This model outperforms the baseline for 53\%, 80\% and 44\% of the Robust04, Core17 and Core18 queries respectively.
...
$ \textrm{BERT}(\textrm{MS MARCO}) $ does not seem to be able to match helpful terms as well on Robust04 or Core18; the top sentences as judged by this model are mostly irrelevant for even the best performing queries on Robust04 and Core18.
...
\midrule
\textbf{$ \textrm{BERT}(\textrm{MS MARCO}\rightarrow\textrm{MB}) $} & 0.4250$^{\dagger}$ & 0.3865$^{\dagger}$ & 0.3505$^{\dagger}$ & 0.4035$^{\dagger}$ \\
\bottomrule
\end{tabular}
}
\caption{Average AP with respect to query length on Robust04.}
...
For some other models, the decrease in AP reaches a minimum at query length 3.
...
\midrule
\textbf{$ \textrm{BERT}(\textrm{MS MARCO}\rightarrow\textrm{MB}) $} & 0.3697$^{\dagger}$ & 0.4657$^{\dagger}$ & 0.5324$^{\dagger}$ \\
\bottomrule
\end{tabular}
}
...
However, the change in document length results in an increase of almost 0.03 AP for $ \textrm{BERT}(\textrm{MS MARCO}^{\star}) $ ($ \textrm{2S} $), thus approaching $ \textrm{BERT}(\textrm{MB}) $ in all metrics.
...
\textbf{BM25+RM3} & 0.2903 & 0.3821 & 0.4407 \\
\midrule
\textbf{2S: $ \textrm{BERT} (\textrm{MB}) $} & 0.3031$^{\dagger}$ & 0.4014$^{\dagger}$ & 0.4580$^{\dagger}$ \\
\midrule
\textbf{1S: $ \textrm{BERT} (\textrm{CAR}) $} & 0.2959 & 0.3936 & 0.4480 \\
\midrule
\textbf{1S: $ \textrm{BERT} (\textrm{MS MARCO}) $} & 0.2962 & 0.3936 & 0.4483 \\
\midrule
\textbf{1S: $ \textrm{BERT} (\textrm{CAR}\rightarrow\textrm{MB}) $} & 0.3037$^{\dagger}$ & 0.3998$^{\dagger}$ & 0.4527 \\
\midrule
\textbf{2S: $ \textrm{BERT} (\textrm{MS MARCO}\rightarrow\textrm{MB}) $} & 0.3101$^{\dagger}$ & 0.4102$^{\dagger}$ & 0.4639$^{\dagger}$ \\
\bottomrule
\end{tabular}
}
\caption{Retrieval effectiveness on pruned Robust04 which only includes sentences that do not contain any query terms. }
...
To isolate the contribution of BERT, we filter sentences in Robust04 that do not contain any of the query terms.
...
As expected, filtering Robust04 sentences leads to a decrease in all metrics across all models, which confirms that exact matching signals are still valuable in relevance predictions over long documents.
